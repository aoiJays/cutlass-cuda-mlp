...
void cutlassMatrixMul_trans(float * d_A, float * d_B, float * d_C, int M, int N, int K) {
    ...
    using Gemm = cutlass::gemm::device::Gemm<
        float, cutlass::layout::ColumnMajor, // 数据类型和布局类型 A^T
        float, cutlass::layout::RowMajor,    // 数据类型和布局类型 B
        float, cutlass::layout::RowMajor,    // 数据类型和布局类型 C
        float                                // 累加器类型
    >;
    ...
    // 设置 GEMM 操作参数
    cutlass::gemm::GemmCoord problem_size(K, N, M); // 注意顺序为 K, N, M
    float alpha = 1.0f;
    float beta = 0.0f;
    ...
    typename Gemm::Arguments arguments{
        problem_size,
        {d_A, M}, // A 的 leading dimension 为 M
        {d_B, N}, // B 的 leading dimension 为 N
        {d_C, N}, // C 的 leading dimension 为 N
        {d_C, N}, // C 的 leading dimension 为 N
        {alpha, beta},
    };
